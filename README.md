...
**Назначение:** генерация и обновление раздела `## Skills` в `README.md` по единому шаблону.

- Берет данные в первую очередь из meta/frontmatter блока `SKILL.md`.
- Нормализует описания и переводит их на русский.
- Сохраняет единый формат для всех записей о скиллах.
- Обновляет только целевой раздел README, не ломая остальные части документа.
